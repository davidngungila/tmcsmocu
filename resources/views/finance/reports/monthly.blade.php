...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Financial transactions for {{ \Carbon\Carbon::parse($month)->format('F Y') }}</p>
        </div>
        <div class="flex items-center space-x-3">
            <input type="month" value="{{ $month }}" onchange="window.location.href='{{ route('finance.reports.monthly') }}?month=' + this.value" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
            <a href="{{ route('finance.reports.monthly.pdf', ['month' => $month]) }}" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-bold flex items-center space-x-2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">No transactions found for this month</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
