...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-600 mt-1 text-sm sm:text-base">Manage church events</p>
        </div>
            ...
            <table class="w-full min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-4 text-left text-xs font-bold text-gray-500 uppercase">Type</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-4 text-sm font-bold text-gray-900">{{ $event->title }}</td>
                        <td class="px-4 py-4">
                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-800">
                                {{ ucfirst(str_replace('_', ' ', $event->type)) }}
                            </span>
                        </td>
                        <td class="px-4 py-4 text-sm text-gray-700">{{ $event->start_date->format('M d, Y') }}</td>
                        <td class="px-4 py-4 text-sm text-gray-700">{{ $event->location ?? 'N/A' }}</td>
                        <td class="px-4 py-4">
                            <span class="px-2 py-1 text-xs font-medium rounded-full {{ $event->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $event->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-4 py-4 text-right text-sm font-medium">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('events.show', $event->id) }}" class="text-purple-600 hover:text-purple-900">View</a>
                                <a href="{{ route('events.edit', $event->id) }}" class="text-blue-600 hover:text-blue-900">Edit</a>
                                <form action="{{ route('events.destroy', $event->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                                    @csrf
                                    @method('DELETE')
