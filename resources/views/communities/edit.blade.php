...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Community</h1>
            <p class="text-gray-600 mt-1 text-sm sm:text-base">Update community information</p>
        </div>
        <a href="{{ route('communities.show', $community->id) }}" class="text-gray-600 hover:text-gray-800">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('communities.update', $community->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('description', $community->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="leader_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    ...
                    <option value="{{ $parishioner->id }}" {{ old('leader_id', $community->leader_id) == $parishioner->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="is_active" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    <option value="1" {{ old('is_active', $community->is_active) ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ !old('is_active', $community->is_active) ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
