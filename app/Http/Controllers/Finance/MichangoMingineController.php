...
class MichangoMingineController extends Controller
{
    ...
    public function index()
    {
        ...
        $query = FinanceTransaction::where('type', 'income')
            ->where('category', 'michango_mingine');
        
        if ($activeYear) {
            $query->where('financial_year_id', $activeYear->id);
        }
        
        $michango = $query->with(['creator', 'parishioner'])
            ->latest()
            ->paginate(20);
        
        return view('finance.michango-mingine.index', compact('michango', 'activeYear'));
    }
    ...
    public function create()
    {
        ...
        return view('finance.michango-mingine.create', compact('parishioners'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated['category'] = 'michango_mingine';
        $validated['title'] = 'Contribution - ' . $parishioner->full_name;
        ...
        if ($parishioner->phone || $parishioner->contact_number) {
            ...
            $message = "Thank you {$parishioner->first_name} for your Contribution of TSh {$amount}. God bless you.";
            
            try {
                $this->notificationService->sendSMS($phone, $message);
                Log::info('Mchango SMS sent successfully', [
                    'parishioner_id' => $parishioner->id,
                    'phone' => $phone,
                    'amount' => $validated['amount']
                ]);
            } catch (\Exception $e) {
                Log::error('Failed to send Mchango SMS', [
                    'parishioner_id' => $parishioner->id,
                    'error' => $e->getMessage()
                ]);
            }
    ...
    {
        $mchango = FinanceTransaction::where('type', 'income')
            ->where('category', 'michango_mingine')
            ->with(['creator', 'parishioner'])
            ->findOrFail($id);
        
        return view('finance.michango-mingine.show', compact('mchango'));
    }
}
